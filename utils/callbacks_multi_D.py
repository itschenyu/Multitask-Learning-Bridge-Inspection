...
class EvalCallback():
    ...
    def get_miou_png(self, image, cross_e, cross_d):
        ...
        with torch.no_grad():
            ...
            if self.cuda:
                images = images.cuda()
                cross_e = cross_e.cuda()
                cross_d = cross_d.cuda()

            pr_e = self.net(images, cross_e.narrow(0, 7, 1), cross_d.narrow(0, 7, 1))[0][0]
            pr_d = self.net(images, cross_e.narrow(0, 7, 1), cross_d.narrow(0, 7, 1))[1][0]
    ...
    def on_epoch_end(self, epoch, model_eval, cross_e, cross_d):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for image_id in tqdm(self.image_ids):
                ...
                image_e       = self.get_miou_png(image, cross_e, cross_d)[0]
                image_e.save(os.path.join(pred_dir_e, image_id + ".png"))
                image_d       = self.get_miou_png(image, cross_e, cross_d)[1]
                image_d.save(os.path.join(pred_dir_d, image_id + ".png"))               
                        
            print("Calculate miou.")
            _, IoUs_e, _, _, _, IoUs_d, _, _ = compute_mIoU(gt_dir_e, pred_dir_e, gt_dir_d, pred_dir_d, self.image_ids, self.num_classes, None)
