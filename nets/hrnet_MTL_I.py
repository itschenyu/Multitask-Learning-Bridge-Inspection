...
class HRnet(nn.Module):
    def __init__(self, num_classes = 21, backbone = 'hrnetv2_w18', pretrained = False):
        ...
        self.projection_e = nn.Linear(480, 480)
        self.projection_d = nn.Linear(480, 480)
    ...
    def forward(self, inputs):
        ...
        x_1  = self.projection_d(x)
        x_1  = torch.swapaxes(x_1, 1, 3)
        x_d  = self.last_layer_d(x_1)
        x_1  = F.interpolate(x_d, size=(H, W), mode='bilinear', align_corners=True)

        x    = self.projection_e(x)
        x    = torch.swapaxes(x, 1, 3)
        x = self.last_layer_e(x)
